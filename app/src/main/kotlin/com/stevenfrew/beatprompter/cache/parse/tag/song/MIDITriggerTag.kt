    ...
    init {
        ...
        if (bits.size > 1)
            if (type==TriggerType.SongSelect)
                throw MalformedTagException(this, BeatPrompterApplication.getResourceString(R.string.song_index_must_have_one_value))
        if (bits.size > 4 || bits.isEmpty())
            if (type==TriggerType.SongSelect)
                throw MalformedTagException(this, BeatPrompterApplication.getResourceString(R.string.song_index_must_have_one_value))
            else
                throw MalformedTagException(this, BeatPrompterApplication.getResourceString(R.string.song_index_must_have_one_two_or_three_values))
