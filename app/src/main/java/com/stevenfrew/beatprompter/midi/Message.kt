...
open class Message constructor(bytes:ByteArray)
{
    ...
    companion object {
        ...
        internal val MIDI_SONG_POSITION_POINTER_BYTE = 0xf2.toByte()
        internal val MIDI_SONG_SELECT_BYTE = 0xf3.toByte()
        internal val MIDI_START_BYTE = 0xfa.toByte()
        internal val MIDI_CONTINUE_BYTE = 0xfb.toByte()
        internal val MIDI_STOP_BYTE = 0xfc.toByte()
        internal val MIDI_SYSEX_END_BYTE = 0xf7.toByte()

        internal val MIDI_CONTROL_CHANGE_BYTE = 0xb0.toByte()
        internal val MIDI_PROGRAM_CHANGE_BYTE = 0xc0.toByte()

        internal val MIDI_MSB_BANK_SELECT_CONTROLLER = 0.toByte()
        internal val MIDI_LSB_BANK_SELECT_CONTROLLER = 32.toByte()
    ...
    private fun isSystemCommonMessage(message: Byte): Boolean {
        return mMessageBytes.isNotEmpty() && mMessageBytes[0] == message
    }

    private fun isChannelVoiceMessage(message: Byte): Boolean {
        return mMessageBytes.isNotEmpty() && (mMessageBytes[0] and 0xF0.toByte() == message)
    }
    ...
    internal fun getMIDIChannel(): Byte {
        return mMessageBytes[0] and 0x0F
    }

    internal fun getBankSelectValue(): Int {
        return mMessageBytes[2].toInt()
    }
