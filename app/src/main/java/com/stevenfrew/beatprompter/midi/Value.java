        ...
        if(strVal.equals("*"))
            ...
            try {
                ...
                if(referencedArgIndex<0)
                    errors.add(new FileParseError(lineNumber, BeatPrompterApplication.getResourceString(R.string.not_a_valid_argument_index)));
                else
            ...
            if (argIndex < argCount - 1)
                errors.add(new FileParseError(lineNumber, BeatPrompterApplication.getResourceString(R.string.channel_must_be_last_parameter)));
            try {
                byte channel=Utils.parseByte(strVal.substring(1));
                if((channel<1)||(channel>16))
                    errors.add(new FileParseError(lineNumber,BeatPrompterApplication.getResourceString(R.string.invalid_channel_value)));
                return new ChannelValue(channel);
            } catch (NumberFormatException nfe) {
                errors.add(new FileParseError(lineNumber, BeatPrompterApplication.getResourceString(R.string.not_a_valid_byte_value)));
            }
        } else if (strVal.contains("_")) {
            if(strVal.indexOf("_")!=strVal.lastIndexOf("_"))
                errors.add(new FileParseError(lineNumber,BeatPrompterApplication.getResourceString(R.string.multiple_underscores_in_midi_value)));
            ...
            try
            {
                if(Utils.looksLikeHex(strVal)) {
                    ...
                    if((channelValue&0x0F)!=0)
                        errors.add(new FileParseError(lineNumber, BeatPrompterApplication.getResourceString(R.string.merge_with_channel_non_zero_lower_nibble)));
                    else
                        return new ChanneledCommandValue(channelValue);
                }
                else
                    errors.add(new FileParseError(lineNumber, BeatPrompterApplication.getResourceString(R.string.underscore_in_decimal_value)));
            }
            catch(NumberFormatException nfe)
            {
                errors.add(new FileParseError(lineNumber, BeatPrompterApplication.getResourceString(R.string.not_a_valid_byte_value)));
