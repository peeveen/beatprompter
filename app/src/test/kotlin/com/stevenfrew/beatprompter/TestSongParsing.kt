...
class TestSongParsing {
	...
	private fun getAllFilesInResources(subfolder: String, suffix: String? = null): List<File> {
		val resourcesPath =
			Paths.get(TEST_DATA_FOLDER_PATH.pathString, subfolder)
		return Files.walk(resourcesPath)
			.filter { item -> Files.isRegularFile(item) }
			.filter { item -> suffix.isNullOrBlank() || item.toString().endsWith(suffix) }
	...
	@Test
	fun testSongParsing() {
		getAllFilesInResources("songs", ".txt").forEach {
			val cachedFile =
				CachedFile(it, it.path, it.name, Date(it.lastModified()), listOf(it.parent ?: ""))
